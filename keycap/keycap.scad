...
has_bump = 1; // XXX: Enable homing bump

// TODO: Implement stem for Matias switches.
stem_type = 1; // XXX: Stem shape (CherryMX = 1, Matias/ALPS = 2)
...
module key_shape (key_type){
    difference()
    {
        if (key_type == 2)
        {
            hull()
            {
                roundedRect([base_width*key_size, base_width, .001], base_corner_radius);

                translate([0, DSA_top_skew, DSA_z])
                  roundedRect([base_width*key_size - DSA_top_x_diff,
        ...
        else
        {
            hull()
            {
                roundedRect([base_width*key_size, base_width, .001], base_corner_radius);
                    ...
                    roundedRect([base_width*key_size - DCS_top_x_diff,
    ...
    difference(){
        difference()
        {
            ...
            translate([0, 0, cross_depth/2])
              union()
              {
                  cube([cross_width_x, cross_arm_length, cross_depth], center=true);
                  cube([cross_arm_length, cross_width_y, cross_depth], center=true);
              }
        }
...
module keycap (key_type){
    union()
    {
        ...
        if (stem_type == 1) cherrymx_stem(key_type);
    }
}
